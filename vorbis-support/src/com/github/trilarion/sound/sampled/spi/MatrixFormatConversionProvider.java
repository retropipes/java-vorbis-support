...
public abstract class MatrixFormatConversionProvider
    ...
    private static final Logger LOG = Logger
            .getLogger(MatrixFormatConversionProvider.class.getName());
    /*
     ...
     */
    protected MatrixFormatConversionProvider(List<AudioFormat> sourceFormats,
            List<AudioFormat> targetFormats, boolean[][] abConversionPossible) {
        ...
        for (int nSourceFormat = 0; nSourceFormat < sourceFormats
            ...
            m_targetFormatsFromSourceFormat.put(sourceFormat,
            ...
            for (int nTargetFormat = 0; nTargetFormat < targetFormats
                ...
                if (abConversionPossible[nSourceFormat][nTargetFormat]) {
                    AudioFormat.Encoding targetEncoding = targetFormat
                            .getEncoding();
    ...
    public AudioFormat.Encoding[] getTargetEncodings(AudioFormat sourceFormat) {
        for (Entry<AudioFormat, List<AudioFormat.Encoding>> entry : m_targetEncodingsFromSourceFormat
                .entrySet()) {
    ...
    public AudioFormat[] getTargetFormats(AudioFormat.Encoding targetEncoding,
        ...
        for (Entry<AudioFormat, Map<AudioFormat.Encoding, Collection<AudioFormat>>> entry : m_targetFormatsFromSourceFormat
            ...
            AudioFormat format = entry.getKey();
            if (AudioFormats.matches(format, sourceFormat)) {
                Map<AudioFormat.Encoding, Collection<AudioFormat>> targetEncodings = entry
                        .getValue();
                Collection<AudioFormat> targetFormats = targetEncodings
                        .get(targetEncoding);
