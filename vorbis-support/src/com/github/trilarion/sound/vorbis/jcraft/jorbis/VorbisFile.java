...
public class VorbisFile {
    private static final Logger LOG = Logger
            .getLogger(VorbisFile.class.getName());
    ...
    // return: -1) error
    // 0) OK
    final int open(InputStream is, byte[] initial, int ibytes)
            throws SoundException {
        ...
        if (i < 0) {
            ...
            if (seekable) {
                ...
                return ((int) Math.rint(
                        (offsets[i + 1] - dataoffsets[i]) * 8 / time_total(i)));
            } else {
                ...
                if (vi[i].bitrate_nominal > 0) {
                    ...
                    if (vi[i].bitrate_upper > 0) {
                        if (vi[i].bitrate_lower > 0) {
                            return (vi[i].bitrate_upper + vi[i].bitrate_lower)
                                    / 2;
                        } else {
    ...
    int time_seek(float seconds) {
        ...
        {
            long target = (long) (pcm_total
                    + (seconds - time_total) * vi[link].rate);
